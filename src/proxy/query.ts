...
function computePassthroughMaxTurns(
  ...
  if (singleTurnHandoff && !liftSingleTurnCap) return 1
...
const presetAppend = [append, GIT_STATUS_PROVENANCE_NOTE, REPLAY_PROVENANCE_NOTE].filter(Boolean).join("")
  ...
  if (clientSystemPromptOption) {
    const systemPrompt = append
      ? `${clientSystemPromptOption}\n\n${append}${REPLAY_PROVENANCE_NOTE}`
      : `${clientSystemPromptOption}${REPLAY_PROVENANCE_NOTE}`
    return { systemPrompt }
  }
...
export function buildQueryOptions(ctx: QueryContext, abortController?: AbortController): BuildQueryResult {
  const {
    ...
    repeatClientSystemPromptOnResume, memory, dreaming, sharedMemory, maxBudgetUsd, maxOutputTokens, fallbackModel, sdkDebug, additionalDirectories, advisorModel,
  } = ctx
