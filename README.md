...
- **Proxy-host scratchpad paths** — the Claude CLI may advertise a proxy-host scratchpad directory that clients can't use. Meridian intentionally does not set `CLAUDE_CODE_SESSION_KIND=bg` to suppress it because Claude treats that value as a real background job and prevents the next client tool-result turn from resuming the session.
